...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    //全局变量
    that.data.globalId = options.audiosid;
    wx.request({
      method: 'GET',
      url: Config.restUrl + 'loverecord/loveaudio/' + options.audiosid + '.do',
  ...
  submitComment: function (event) {
    ...
    wx.request({
      ...
      data: {
        'content': commentText, 'userId': 1514, 'loveAudioId': this.data.globalId,
      },
      method: 'POST',
      header: {
  ...
  onUnload: function () {
    ...
    console.log(pretPage, currPage)
    for (var i = 0; i < pretPage.data.userContent.userPublishs.length; i++) {
      if (pretPage.data.userContent.userPublishs[i].type == 'LoveAudio') {
        if (pretPage.data.userContent.userPublishs[i].loveAudios.id == currPage.data.recordItem.id) {
          pretPage.data.userContent.userPublishs[i].loveAudios.commentcount = pretPage.data.userContent.userPublishs[i].loveAudios.commentcount + that.data.commentsize;
          pretPage.data.userContent.userPublishs[i].loveAudios.likecount = currPage.data.recordItem.likecount;
        }
